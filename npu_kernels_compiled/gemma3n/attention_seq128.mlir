...
module @gemma3n_attention_seq128 {
  // Model parameters
  ...
  func.func @attention_forward(
    %hidden_states: tensor<1x128x1536xi8>,
    ...
    %q_int32 = linalg.matmul ins(%hidden_states, %q_weight : 
      tensor<1x128x1536xi8>, 
      tensor<1536x1536xi8>) 
      outs(%q_out : tensor<1x128x1536xi32>)
      
    %k_int32 = linalg.matmul ins(%hidden_states, %k_weight :
      tensor<1x128x1536xi8>,
      tensor<1536x1536xi8>)
      outs(%k_out : tensor<1x128x1536xi32>)
      
    %v_int32 = linalg.matmul ins(%hidden_states, %v_weight :
      tensor<1x128x1536xi8>,
      tensor<1536x1536xi8>)
      outs(%v_out : tensor<1x128x1536xi32>)
    ...
    %q_heads = tensor.reshape %q_fp16 : 
      tensor<1x128x1536xf16> to 
      tensor<1x128x12x128xf16>
    ...
    %scores = linalg.batch_matmul_transpose_b ins(%q_heads, %k_expanded :
      tensor<1x128x12x128xf16>,
      tensor<1x128x12x128xf16>)
      outs(%score_out : tensor<1x12x128x128xf16>)
    ...
    %attention_output = linalg.batch_matmul ins(%attention_weights, %v_expanded :
      ...
      tensor<1x128x12x128xf16>)
      outs(%attn_out : tensor<1x128x12x128xf16>)
    
    // Reshape and output projection
    %output_2d = tensor.reshape %attention_output :
      tensor<1x128x12x128xf16> to
      tensor<1x128x1536xf16>
    ...
    return %output_i8 : tensor<1x128x1536xi8>
  }
