#!/usr/bin/env python3
"""
Qwen2.5-7B NPU+iGPU Hybrid Execution
Main script for running Qwen2.5-7B with AMD NPU Phoenix + Radeon 780M hybrid execution
...
def print_banner():
    ...
    banner = """
╔═══════════════════════════════════════════════════════════════════════════════╗
║                    🚀 Qwen2.5-7B Hybrid NPU+iGPU Execution                    ║
║                                                                               ║
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Run Qwen2.5-7B with hybrid NPU+iGPU execution')
    
    # Model configuration
    parser.add_argument('--model', type=str, default='Qwen/Qwen2.5-7B-Instruct',
                       help='Model ID or path (default: Qwen/Qwen2.5-7B-Instruct)')
    ...
    parser.add_argument('--igpu-memory', type=int, default=12288,
                       help='iGPU memory budget in MB (default: 12288)')
...
def configure_logging(verbose: bool):
    ...
    if verbose:
        ...
        logging.getLogger('qwen25_loader').setLevel(logging.DEBUG)
...
async def main():
    ...
    try:
        # Initialize loader
        print("\n🔄 Initializing Qwen2.5-7B loader...")
        loader = Qwen25Loader(hybrid_config)
