...
module @gemma3_27b_attention_seq128 {
  // Model parameters
  ...
  func.func @attention_forward(
    %hidden_states: tensor<1x128x4608xi8>,
    ...
    %q_heads = tensor.reshape %q_fp16 : 
      tensor<1x128x4608xf16> to 
      tensor<1x128x48x96xf16>
      
    %k_heads = tensor.reshape %k_fp16 :
      tensor<1x128x768xf16> to
      tensor<1x128x8x96xf16>
    ...
    %scores = linalg.batch_matmul_transpose_b ins(%q_heads, %k_expanded :
      tensor<1x128x48x96xf16>,
      tensor<1x128x48x96xf16>)
      outs(%score_out : tensor<1x48x128x128xf16>)
    ...
    %attention_output = linalg.batch_matmul ins(%attention_weights, %v_expanded :
      tensor<1x48x128x128xf16>,
      tensor<1x128x48x96xf16>)
      outs(%attn_out : tensor<1x128x48x96xf16>)
    
    // Reshape and output projection
    %output_2d = tensor.reshape %attention_output :
      tensor<1x128x48x96xf16> to
      tensor<1x128x4608xf16>
    ...
    return %output_i8 : tensor<1x128x4608xi8>
  }
